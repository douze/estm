...
func test_get_coefficients() -> void:
	assert_eq(wavefunction.get_coefficients(Vector2(0, 0)), ['C', 'L', 'S'])
	wavefunction._coefficients[1][2].pop_back()
	assert_eq(wavefunction.get_coefficients(Vector2(2, 1)), ['C', 'L'])
...
func test_compute_shannon_entropy() -> void:
	assert_almost_eq(wavefunction.compute_shannon_entropy(Vector2(0, 0)), 1.060857, 0.0000001)
...
func test_collapse() -> void:
	wavefunction.collapse(Vector2(2, 3))
	assert_eq(wavefunction.get_coefficients(Vector2(2, 3)).size(), 1)


func test_constain() -> void:
	wavefunction.constrain(Vector2(1, 2), 'S')
	assert_eq(wavefunction.get_coefficients(Vector2(1, 2)), ['C', 'L'])
